...
THEME_NAME=TichPhong
COLOR_VARIANTS=('' '-light' '-dark')
SIZE_VARIANTS=('' '-compact')
THEME_VARIANTS=('-zen' '-jade' '-mystic' '-cinnabar' '-purple' '-tet')
...
install() {
  ...
  cp -r ${SRC_DIR}/gtk-2.0/assets/tichphong${theme}/assets${ELSE_DARK}                      ${THEME_DIR}/gtk-2.0/assets

  if [[ ${color} == '' ]] && [[ ${theme} == '' ]]; then
    cp -r ${SRC_DIR}/gtk-2.0/assets/tichphong/assets-dark-zen/pathbar_button_active.png    ${THEME_DIR}/gtk-2.0/assets
    cp -r ${SRC_DIR}/gtk-2.0/assets/tichphong/assets-dark-zen/pathbar_button_prelight.png  ${THEME_DIR}/gtk-2.0/assets
  fi
    ...
    -tweaks|--tweaks)
      ...
      for tweaks in "$@"; do
        case "$tweaks" in
          zen)
            zen="true"
            echo -e "Install zen windows button version ..."
            shift
            ;;
          flat)
            flat="true"
          ...
          mix)
            mix="true"
            echo -e "Install mix dark zen version ..."
            shift
            ;;
          translucent)
            translucent="true"
...
install_theme_color() {
  if [[ "$theme" != '-jade' ]]; then
    ...
    sed -i "/\$theme:/s/jade/${theme_color}/" ${SRC_DIR}/_sass/_tweaks-temp.scss
  fi
}
